...
import org.example.postapi.domain.image.exception.ImageNotFoundException;
...
import static org.example.postapi.domain.image.ImageConstants.*;
import static org.example.postapi.domain.image.ImageSpecifications.*;
...
public class ImageServiceImpl implements ImageService{
    ...
    public Page<ImageDto> getImages(UUID userId, Pageable pageable){
        ...
        // Non-deleted & Own
        var spec = isNotDeleted().and(ownedBy(AppUser.builder().id(userId).build()));
    ...
    @Override
    @Cacheable(value=SHORT_LIVE_CACHE_NAME, key = "'image:own'+ #userId + ':' + #imageId")
    public boolean isOwner(Long imageId, UUID userId){
        AppUser owner = AppUser.builder().id(userId).build();
        var spec = isNotDeleted().and(idEquals(imageId)).and(ownedBy(owner));
